 ...
 */
public class AnterosInstagramSessionManager {
    ...
    private OnLoginInstagramListener onLoginListener;
    private OnLogoutInstagramListener onLogoutListener;

    private final static String TAG = AnterosInstagramSessionManager.class.getName();
    static AnterosInstagramConfiguration configuration;

    private WeakReference<FragmentActivity> activity;


    public AnterosInstagramSessionManager(FragmentActivity activity, String clientId, String clientSecret, String redirectURL, String scope) {

        mSharedPreferences = activity.getSharedPreferences(SHARED_PREFERENCES_NAME, Context.MODE_PRIVATE);
    ...
    public void setOnLoginListener(OnLoginInstagramListener onLoginListener) {
        this.onLoginListener = onLoginListener;
    }

    public void setOnLogoutListener(OnLogoutInstagramListener onLogoutListener) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (uri != null && uri.toString().startsWith(mRedirectURL)) {
            ...
            onLoginListener.onFail(new AnterosInstagramException("Incorrect URI returned " + uri));
        }
    }
    ...
    public void login() {
        Intent intent = new Intent(activity.get(), AnterosOAuthActivity.class)
                .putExtra(AnterosOAuthActivity.PARAM_CALLBACK, mRedirectURL)
                .putExtra(AnterosOAuthActivity.PARAM_URL_TO_LOAD, mAuthURLString);
    ...
    public void setOnLoginInstagramListener(OnLoginInstagramListener onLoginInstagramListener) {
        this.onLoginListener = onLoginInstagramListener;
    }

    public void setOnLogoutInstagramListener(OnLogoutInstagramListener onLogoutInstagramListener) {
        this.onLogoutListener = onLogoutInstagramListener;
    }
    ...
    public void getProfile(OnProfileInstagramListener onProfileInstagramListener) {
        ...
        new RequestProfileAsyncTask(onProfileInstagramListener).execute(args);
    }
    ...
    private InstagramProfile parseResponse(JSONObject jsonResponse) throws JSONException {
        ...
        if(jsonResponse.has("id")) {
            profile.setId(jsonResponse.getString("id"));
        }
    ...
    private boolean checkTokenError(Bundle result) {
        if (result != null && result.containsKey(ERROR_CODE) && result.getString(ERROR_CODE).contains("400") && result.getString(ERROR_CODE).contains("OAuth")) {
            ...
            mRequestBundle.remove(AnterosInstagramAsyncTask.RESULT_ERROR);
            login();
            return true;
        }
        return false;
    ...
    private class RequestProfileAsyncTask extends AnterosInstagramAsyncTask {
        ...
        private final OnProfileInstagramListener listener;

        public RequestProfileAsyncTask(OnProfileInstagramListener listener) {
